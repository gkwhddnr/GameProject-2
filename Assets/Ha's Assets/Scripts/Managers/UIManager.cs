...
/// <summary>
/// UIManager - UI 전용 씬(Choi_UIRoot)을 Additive 방식으로 로드하여 관리
/// EventSystem 중복 처리는 EventSystemManager가 자동으로 처리
/// </summary>
public class UIManager : MonoBehaviour
{
    ...
    private IEnumerator LoadUISceneAsync()
    {
        ...
        Debug.Log($"[UIManager] UI 씬 로드 완료: {uiSceneName}");
    }
    ...
    private bool IsSceneInBuildSettings(string sceneName)
    {
        ...
        Debug.LogError($"[UIManager] '{sceneName}' 씬이 Build Settings에 없습니다!");
        return false;
    }

    public bool IsUISceneLoaded()
    {
        return isUISceneLoaded;
    }

    void OnDestroy()
    {
        if (isUISceneLoaded && Instance == this)
